...
// serial_packet[ 7]  -> 0x89 (reserved)
// serial_packet[ 8]  -> PACKET_ID
// serial_packet[ 9]  -> NUMBER OF FRAMES (LS Byte)
// serial_packet[10]  -> NUMBER OF FRAMES (MS Byte)
// serial_packet[11]  -> FRAME NUMBER (LS Byte)
// serial_packet[12]  -> FRAME NUMBER (MS Byte)
// serial_packet[13]  -> First byte of the frame
// .....
...
class DataTransmission
{
	...
	int sendPacketRADIO(serial_source src);

public:

	int sendPacketUDP(string server_addr, int port, int imgID, int nFeats, vector<uchar> kpts_bitstream, vector<uchar> feats_bitstream, char* rec_stream);

	serial_source openSerialRADIO(const char *serial_device, int baud_rate, int non_blocking);

	int sendImgInfoRADIO(serial_source src, int imgID, bool atc_cta,
						 bool compressed = true,
						 string descName = "BRISK", int nFeats = 0,
						 int imWidth = 640, int imHeight = 480);

	int sendNoObjPktRADIO(serial_source src);

	int sendFeaturesRADIO(serial_source src, int imgID, int nFeats, vector<uchar> kpts_bitstream, vector<uchar> feats_bitstream);
	int sendJpegRADIO(serial_source src, int imgID, vector<uchar> jpeg_bitstream);
	int sendACK_RADIO(serial_source src, ackType at, int imgID, actions *acts = NULL);
	int sendCMD_RADIO(serial_source src, commandType ct, actions *acts = NULL);
	int reqACK_RADIO (serial_source src, ackType at, int imgID);

	bool waitACK_RADIO(serial_source src, ackType at, int imgID, double timeout = 0, actions *acts = NULL);
